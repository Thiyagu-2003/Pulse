...
class PermissionController : PermissionManagerInterface,
    ...
    private var permissions: Array<String> =
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            // PULSE PATCH: reading songs needs only READ_MEDIA_AUDIO. Also
            // demanding READ_MEDIA_IMAGES meant a user who allowed "Music and
            // audio" was still treated as having no permission.
            arrayOf(Manifest.permission.READ_MEDIA_AUDIO)
        } else {
            // PULSE PATCH: WRITE_EXTERNAL_STORAGE isn't needed to read, and
            // Pulse doesn't declare it, so it could never be granted.
    ...
    override fun retryRequestPermission() {
        ...
        // PULSE PATCH: the list can hold a single permission now.
