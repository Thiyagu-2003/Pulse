...
class MiniPlayer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Swipe sideways to change song, up to open Now Playing — the gestures
    // every music app's mini player has.
    return GestureDetector(
      onHorizontalDragEnd: (d) {
        final v = d.primaryVelocity ?? 0;
        if (v < -300) playerProvider.skipToNext();
        if (v > 300) playerProvider.skipToPrevious();
      },
      onVerticalDragEnd: (d) {
        if ((d.primaryVelocity ?? 0) < -300) _openNowPlaying(context);
      },
      child: Padding(
        ...
        child: GlassContainer(
          ...
          child: _content(context, playerProvider, track),
        ),
      ),
    );
  }
    ...
    Navigator.push(
      ...
      PageRouteBuilder(
        ...
        pageBuilder: (_, animation, secondaryAnimation) =>
            Theme(data: AppTheme.darkTheme, child: const NowPlayingScreen()),
      ...
      children: [
        Row(
          children: [
            ...
            // Controls: previous, play/pause (with a ring while the
            // song is loading), next.
        ...
        StreamBuilder<Duration>(
          ...
          builder: (context, snapshot) {
            final position = (snapshot.data ?? Duration.zero).inMilliseconds
                .toDouble();
            final duration =
                playerProvider.currentDuration?.inMilliseconds.toDouble() ??
                1.0;
